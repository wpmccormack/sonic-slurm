...
options.register("address", "", VarParsing.multiplicity.list, VarParsing.varType.string, "server addresses")
#options.register("port", 8001, VarParsing.multiplicity.singleton, VarParsing.varType.int, "server port")
options.register("port", "8001", VarParsing.multiplicity.list, VarParsing.varType.string, "server port")
...
if len(options.params)>0:
    ...
    options.port = pdict["port"]
    print("server = "+options.address+":"+options.port)
...
if options.sonic:
    ...
    if len(options.address)>0:
        numServ = 0
        #for ad in options.address:
        for ad in range(len(options.address)):
            process.TritonService.servers.append(
                cms.PSet(
                    name = cms.untracked.string(options.serverName+str(numServ)),
                    address = cms.untracked.string(options.address[ad]),
                    port = cms.untracked.uint32(int(options.port[ad])),
